All tests are run under Ubuntu Linux 14.04 and SBCL 1.2.8, with AMD Phenom II X6 2.8GHz and 4GB Memory.
* Test Domains

+ Fibonacci :: Computes a large Fibonacci number. I didnt apply the memoization technique: it is a quite basic implementation.
+ Gomoku :: Matches a set of randomly generated 5x5 field of Gomoku table game against the /winning patterns/ to determine if a Black player is winning, or a White player is winning. 100000 fields are generated and fixed, then supplied to each matcher.
...
Results varies across the test domains, but the performance of trivia+emilie2006 is interesting especially in string-match. Interestingly, trivia (w/o optimization) is also fast in string-match.
