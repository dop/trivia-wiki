...
value. Consider writing a derived pattern that matches against both of a form =(quote string)=
and a form =(quote (string *))= . It has a subpattern =length=, which should be bound to a symbol =*= even when the input is =(quote string)=. With =<>= pattern, it can be implemented as below.
